...
#define AD9238_CAP_RESULT_MAGIC 0x43415031u
#define AD9238_CAP_RESULT_VALID 0x00000001u
...
/*
 * Final capacitance value exported for firmware/J-Link use.
 *
 * The bridge is treated as a series model Z = ESR - j/(2*pi*f*Cs).
 * series_capacitance_pf is valid only when valid_mask contains
 * AD9238_CAP_RESULT_VALID (measured reactance is capacitive: Im(Z) < 0).
 */
typedef struct {
  uint32_t magic;
  uint32_t sequence;
  ...
  float series_capacitance_pf;
  float series_capacitance_nf;
  float equivalent_series_resistance_ohm;
  float dissipation_factor;
} AD9238_CapacitanceResult;
...
extern volatile AD9238_CapacitanceResult g_ad9238_capacitance_result;
