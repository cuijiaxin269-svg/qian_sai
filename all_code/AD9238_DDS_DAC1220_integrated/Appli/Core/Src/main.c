...
#define AD9238_EXPECTED_CHANNEL_B_HZ 1000000.0f
#define DAC1220_TO_ADC_SETTLING_TIME_MS 1000u
...
int main(void)
{
  ...
  g_boot_stage = 7u;
  /*
   * DAC1220_Init() has a bounded calibration wait.  Even if DAC status
   * readback fails, startup continues so ADC/DDS acquisition is not lost.
   * Inspect g_dac1220_init_result and g_dac1220_last_status through J-Link.
   */
  (void)DAC1220_Init();
  HAL_Delay(500u);
  /* Program the requested DC bias before starting the one-shot ADC frame. */
  DAC1220VolWrite(2.00f);

  /*
   * Allow the HP/LP AC-coupling capacitors and analog front-end bias points
   * to settle completely.  Capturing before this delay would mix the charge
   * transient into the 1 MHz DFT and corrupt phase, ESR and capacitance.
   */
  HAL_Delay(DAC1220_TO_ADC_SETTLING_TIME_MS);
  ...
  DDS_AD9833_Init();
  /*
   * AD9833 writes and the analog front-end need time to settle after startup.
   * Do not start the first PSSI/DMA frame until the 1 MHz sine output is
   * stable, otherwise the transient could be processed as valid ADC data.
   */
  HAL_Delay(DDS_AD9833_SETTLING_TIME_MS);
  g_boot_stage = 10u;
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(DDS_SYNC_GPIO_Port, DDS_SYNC_Pin, GPIO_PIN_SET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOE, DDS_CLK_Pin|DDS_DATA_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pin : DDS_SYNC_Pin */
  GPIO_InitStruct.Pin = DDS_SYNC_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
  HAL_GPIO_Init(DDS_SYNC_GPIO_Port, &GPIO_InitStruct);

  /*Configure GPIO pins : DDS_CLK_Pin DDS_DATA_Pin */
  ...
  /* DAC1220 configures PF1/PB5/PC10 itself after MX_PSSI_Init().  In
   * particular, PSSI releases PB5 during its MSP initialization. */
