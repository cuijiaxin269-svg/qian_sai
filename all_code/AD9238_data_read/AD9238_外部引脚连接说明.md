...
PSSI 采集时钟直接使用 AD9238 的 `DCO`，接入 `PF8 / PSSI_PDCK`。
...
当前应用代码在 `Appli/Core/Src/main.c` 中手工初始化了 `USART2`，用于把每帧 AD9238 数据发送给上位机。注意：这部分是代码手工配置，不是 `.ioc` 中的 CubeMX 串口外设配置。
...
原来的 `PA8 / TIM1_CH1` 不再启动，也不再作为 ADC 采样时钟。
`AD9238 DCO -> PF8 / PSSI_PDCK` 用于 STM32 的 PSSI 采集时钟；PA15/TIM2 已停用。

发送格式为文本 CSV，每次 DMA 采满一帧后发送：
...
`ch_a` 和 `ch_b` 已经由软件从 AD9238 交替输出的原始数据中拆分。如果实际示波器/信号源验证发现 A/B 反了，把 `main.c` 中：
