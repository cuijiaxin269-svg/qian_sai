...
void HAL_PSSI_MspInit(PSSI_HandleTypeDef* hpssi)
{
  ...
  */
    ...
    /* Select exactly one candidate pad for PSSI_D10. */
    ...
    */
...
#if defined(AD9238_D10_PB5_PULLUP_DIAGNOSTIC)
    /* A floating PB5 is forced high. If PSSI bit 10 remains low, the fault is
       beyond the PCB/PD6 path. PD6 remains a GPIO input so its external
       activity can still be observed by the runtime diagnostic. */
    ...
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
    /* STM32H7R7 alternate-function table: PD6 PSSI_D10 is AF9.
       AF6 on PD6 is SAI1_SCK_A and leaves PSSI D10 stuck low. */
    GPIO_InitStruct.Alternate = GPIO_AF9_PSSI;
    HAL_GPIO_Init(GPIOD, &GPIO_InitStruct);
#endif
    ...
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
    GPIO_InitStruct.Alternate = GPIO_AF8_PSSI;
    HAL_GPIO_Init(GPIOF, &GPIO_InitStruct);
    ...
    handle_GPDMA1_Channel15.Init.Request = GPDMA1_REQUEST_PSSI;
    ...
    __HAL_LINKDMA(hpssi, hdmarx, handle_GPDMA1_Channel15);
...
void HAL_PSSI_MspDeInit(PSSI_HandleTypeDef* hpssi)
{
  if(hpssi->Instance==PSSI)
  {
    ...
    */
    ...
    HAL_GPIO_DeInit(GPIOF, GPIO_PIN_3|GPIO_PIN_2|GPIO_PIN_4|GPIO_PIN_5
                          |GPIO_PIN_8|GPIO_PIN_11);

    HAL_GPIO_DeInit(GPIOG, GPIO_PIN_3|GPIO_PIN_15);
