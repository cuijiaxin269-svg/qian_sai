...
MCU_POWER_OCP = 0.2
FIXED_BRIDGE_FREQ_HZ = 1_000_000
FIXED_BRIDGE_FREQ_LABEL = "1 MHz"
CV_MIN_VOLTAGE = 0.1
...
# ==========================================
# 模块一：底层设备控制类封装 (Class Definitions)
# ==========================================
...
class TH1992_Controller:
    ...
    def power_mcu_channel(self, voltage=MCU_POWER_VOLTAGE, ocp=MCU_POWER_OCP):
        """给控制板供电通道上电，保证随后可以连接 RS232 通道切换模块。"""
    ...
    def close(self):
        if self.ser and self.ser.is_open:
            self.send_cmd("OUTPut1:STATe OFF")  # 关断测量输出保护
            self.send_cmd("OUTPut2:STATe OFF")  # 关断备用输出保护
            self.ser.close()
...
class TH2840_Controller:
    ...
    def init_instrument(self):
        ...
        cmds = [
            "APERture FAST",  # 速度快速
            "FUNCtion:IMPedance:RANGe:AUTO ON",  # 阻抗量程自动
            "FUNCtion:DCR:RANGe:AUTO ON",  # DC量程自动
            f"FREQuency {FIXED_BRIDGE_FREQ_HZ}",  # 固定测试频率
...
# ==========================================
st.set_page_config(page_title="嵌入式设计大赛测控平台", layout="wide")
...
if 'control_board_serial_connected' not in st.session_state: st.session_state.control_board_serial_connected = False
if 'data_cache' not in st.session_state:
    st.session_state.data_cache = pd.DataFrame(columns=["Time", "Cp", "Rs", "V1", "I1"])
...
with st.sidebar:
    ...
    st.subheader("自制源表配置")
    port_1992 = st.text_input("源表串口号", value="COM13")
    baud_1992 = st.selectbox("源表波特率", [9600, 115200], index=1, key="b1")
    ...
    st.subheader("自制电桥配置")
    port_2840 = st.text_input("电桥串口号", value="COM12")
    baud_2840 = st.selectbox("电桥波特率", [9600, 115200], index=1, key="b2")
    ...
    st.subheader("🔀 通道切换模块配置")
    port_relay = st.text_input("切换模块串口号", value="COM14")
    baud_relay = st.selectbox("切换模块波特率", [9600, 115200], index=1, key="b3")

    if st.session_state.relay is None:
        if st.button("🟢 连接通道切换模块"):
            try:
                ...
                if relay_reply_ok(reply, '8'):
                    ...
                    st.success("通道切换模块连接成功！（初始化握手通过）")
                    st.rerun()
                else:
                    relay.close()
                    st.error(f"连接验证失败：收到'{reply}'")
            except Exception as e:
                st.error(f"连接失败: {e}")
    else:
        st.success("🟢 通道切换模块已连接")
        if st.button("🔴 断开通道切换模块"):
...
# 模块四：主工作区 - 四大功能 Tabs
# ==========================================
st.title("🎛️ 嵌入式设计大赛 - 功率器件参数测控平台")
st.subheader("测量前请先完成系统连接与参数配置")
st.subheader("异常情况下请先停止输出，再断开连接")


tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs([
    "⚡ 功能一：系统初始化", "🎛️ 功能二：参数设置", "📊 功能三：数据采集",
    "📈 功能四：自动扫描", "🏁 功能五：综合测量", "🔀 功能六：通道切换"
])
...
with tab1:
    st.write("### 控制板串口连接")
    serial_col1, serial_col2, serial_col3 = st.columns([2, 2, 1])
    with serial_col1:
        st.text_input("控制板串口号", value="COM11", key="control_board_port")
    with serial_col2:
        st.selectbox("控制板波特率", [9600, 115200], index=1, key="control_board_baud")
    with serial_col3:
        st.write("")
        if st.button("🟢 连接控制板", key="control_board_connect_btn", use_container_width=True):
            st.session_state.control_board_serial_connected = True

    if st.session_state.control_board_serial_connected:
        st.success(
            f"控制板串口连接成功：{st.session_state.control_board_port} @ "
            f"{st.session_state.control_board_baud}"
        )
    ...
    with col1:
        st.subheader("自制源表初始化")
        if st.button("🚀 执行自制源表一键配置"):
            if st.session_state.th1992:
                st.session_state.th1992.init_instrument()
                st.success("自制源表已初始化，测量偏压输出已就绪。")
            else:
                st.warning("请先在左侧连接自制源表")

    with col2:
        st.subheader("自制电桥初始化与校准")
        if st.button("🚀 执行自制电桥一键配置"):
            if st.session_state.th2840:
                st.session_state.th2840.init_instrument()
                st.success("自制电桥基础设置已下发！")
            else:
                st.warning("请先在左侧连接自制电桥")

        st.write("### 🎯 固定频率校准")

        spot_n = 1
        spot_freq = FIXED_BRIDGE_FREQ_HZ
        st.metric("固定测量频率", FIXED_BRIDGE_FREQ_LABEL)

        # 先应用频率和开关状态
        col_apply, _, _ = st.columns([1, 2, 2])
        with col_apply:
            if st.button("📝 应用固定校准设置", use_container_width=True):
                if st.session_state.th2840:
                    st.session_state.th2840.spot_freq(spot_n, spot_freq)
                    st.session_state.th2840.spot_stat(spot_n, 1)
                    st.success(f"校准点 {spot_n}: {FIXED_BRIDGE_FREQ_LABEL}, 已启用")
                else:
                    st.warning("请先连接自制电桥")
        ...
        with c1:
            if st.button("🔓 执行单点开路校准", use_container_width=True):
                if st.session_state.th2840:
                    with st.spinner(f"正在对校准点 {spot_n} 执行开路校准，请稍候..."):
                        result = st.session_state.th2840.spot_open(spot_n, ack=True)
                    if result == '1':
                        st.success(f"校准点 {spot_n} ({FIXED_BRIDGE_FREQ_LABEL}) 开路校准成功！")
                    else:
                        st.error(f"校准点 {spot_n} 开路校准失败（返回: {result}）")
                else:
                    st.warning("请先连接自制电桥")
        with c2:
            if st.button("🔒 执行单点短路校准", use_container_width=True):
                if st.session_state.th2840:
                    with st.spinner(f"正在对校准点 {spot_n} 执行短路校准，请稍候..."):
                        result = st.session_state.th2840.spot_short(spot_n, ack=True)
                    if result == '1':
                        st.success(f"校准点 {spot_n} ({FIXED_BRIDGE_FREQ_LABEL}) 短路校准成功！")
                    else:
                        st.error(f"校准点 {spot_n} 短路校准失败（返回: {result}）")
                else:
                    st.warning("请先连接自制电桥")
...
with tab2:
    ...
    with col1:
        st.subheader("自制源表输出参数")

        v1 = st.number_input("测量电压 (V)", min_value=0.0, max_value=100.0, value=0.0, key="v1")
        ocp1 = st.number_input("限流保护 (A)", min_value=0.001, max_value=1.0, value=0.1, key="ocp1")

        if st.button("应用自制源表参数并开启输出"):
            if st.session_state.th1992:
                # 配置并开启测量偏压输出
                ...
                st.success("测量电压参数下发成功，已开启输出！")
            else:
                st.warning("请先连接自制源表")
    with col2:
        st.subheader("自制电桥测试条件")
        test_freq = FIXED_BRIDGE_FREQ_HZ
        st.metric("固定测试频率", FIXED_BRIDGE_FREQ_LABEL)
        if st.button("应用自制电桥参数"):
            if st.session_state.th2840:
                st.session_state.th2840.send_cmd(f"FREQuency {test_freq}")
                st.success(f"已设定固定频率: {FIXED_BRIDGE_FREQ_LABEL}")
            else:
                st.warning("请先连接自制电桥")
...
with tab3:
    ...
    if st.button("📥 执行单次测量"):
        ...
        if not st.session_state.data_cache.empty:
            ...
            st.write("### 📜 历史测量数据")
            columns_to_show = ["Time", "Cp (pF)", "Rs (Ω)", "测量电压 (V)", "测量电流 (A)"]
...
def _render_cv_chart(saved_curves, live_circuit, live_df):
    ...
    if not has_data:
        fig.add_annotation(
            text="暂无 C-V 数据<br>请在通道切换页切换回路后开始扫描",
    ...
    if is_running and scan_state.get("chart_xlim"):
        ...
        xlim = _cv_log_limits(all_x, pad=1.16, floor=CV_MIN_VOLTAGE) or (CV_MIN_VOLTAGE, 40.0)
    xlim = (max(xlim[0], CV_MIN_VOLTAGE), max(xlim[1], CV_MIN_VOLTAGE * 10))
...
def _cv_scan_worker(scan_state, th1992, th2840, active_circuit, volt_points, delay_t, freq_hz):
    rows = []
    scan_state.update({
        "running": True,
        "done": False,
...
def _start_cv_scan(active_circuit, volt_points, delay_t, freq_hz):
    ...
    scan_state = {
        ...
        "chart_xlim": _cv_log_limits(volt_points, pad=1.18, floor=CV_MIN_VOLTAGE) or (CV_MIN_VOLTAGE, 40.0),
        "chart_ylim": None,
    }
    worker = threading.Thread(
        target=_cv_scan_worker,
        args=(scan_state, st.session_state.th1992, st.session_state.th2840,
              active_circuit, volt_points, delay_t, freq_hz),
        daemon=True
    )
...
def _generate_volt_points(start_v, stop_v):
    """自适应采样电压点: 0.1–20V 区域密集 100 点, 20–100V 区域稀疏 100 点.
    实际扫描范围决定从两个池中各取多少点. 至少保证 3 个点."""
    start_v = max(float(start_v), CV_MIN_VOLTAGE)
    stop_v = float(stop_v)
    if start_v >= stop_v:
...
with tab4:
    ...
    st.subheader("使用前请先在功能二完成源表参数配置")
    st.markdown(f"控制 **自制源表** 提供步进直流偏压，并使用 **自制电桥** 以 **{FIXED_BRIDGE_FREQ_LABEL}** 固定频率同步测量电容 (Cp)。")

    # ==== 参数输入区 ====
    if 'cv_start_v' not in st.session_state:
        st.session_state.cv_start_v = CV_MIN_VOLTAGE
        st.session_state.cv_stop_v = 30.0
        st.session_state.cv_delay_t = 1.0
    st.session_state.cv_start_v = max(float(st.session_state.cv_start_v), CV_MIN_VOLTAGE)

    col1, col2, col3, col4 = st.columns(4)
    with col1:
        st.session_state.cv_start_v = st.number_input(
            "起始电压 (V)", min_value=CV_MIN_VOLTAGE, max_value=100.0,
            value=st.session_state.cv_start_v, step=0.1, key="cv_start_v_key"
        )
    with col2:
        st.session_state.cv_stop_v = st.number_input(
            "终止电压 (V)", min_value=CV_MIN_VOLTAGE, max_value=100.0,
    ...
    st.caption("采样策略：0.1–20 V 密集 (100 点)，20–100 V 稀疏 (100 点)，按实际扫描范围比例分配。")
    ...
    with col_status:
        if st.session_state.cv_active_circuit:
            ...
            st.info("📍 当前测量回路: **未选择**（请先在通道切换页切换回路）")
    with col_clear:
...
# -------- Tab 5: 综合测量 --------
with tab5:
    st.subheader("🏁 功能五：综合参数测量")
    st.markdown("一键切换 Ciss、Coss、Crss、Rg 回路，并读取四个参量。")

    c1, c2, c3 = st.columns(3)
    with c1:
        final_freq = FIXED_BRIDGE_FREQ_HZ
        st.metric("固定测量频率", FIXED_BRIDGE_FREQ_LABEL)
    with c2:
    ...
    def _measure_final_current_param(flow, status_ph=None):
        ...
        # ③ 接通源表通道 1 漏源偏压（继电器已稳定在测量回路，不会带电切换）
        ...
        if flow["phase"] == "prepare":
            ...
            if st.button("已开路，执行开路校准", type="primary", use_container_width=True):
                ...
                try:
                    ...
                    pass
                with st.spinner("正在执行校准点 1 开路校准，等待模块返回完成信号..."):
                ...
                if result == "1":
                    ...
                    st.error(f"开路校准失败或未完成，模块返回: {result}")
            ...
            if st.button("已短路，执行短路校准", type="primary", use_container_width=True):
                ...
                try:
                    ...
                    pass
                with st.spinner("正在执行校准点 1 短路校准，等待模块返回完成信号..."):
                ...
                if result == "1":
                    ...
                    st.error(f"短路校准失败或未完成，模块返回: {result}")
        elif flow["phase"] == "dut":
            st.warning(
                f"{item['name']} 的开路/短路校准已完成。请将 MOSFET 插入底座并确认已经接入当前测量回路，"
                "确认后才会开始读取数据。\n\n⚠️ 自制源表输出已关断，插入 MOSFET 前确保电源已关闭。"
            )
    ...
    if st.button("🚀 一键测量四个参量", type="primary", use_container_width=True):
        if not (st.session_state.th1992 and st.session_state.th2840 and st.session_state.relay):
            st.error("请先连接自制源表、自制电桥和通道切换模块。")
        else:
            # 立即关断源表通道 1，确保后续校准/插拔 MOSFET 安全
            try:
                st.session_state.th1992.send_cmd("SOURce1:VOLTage:LEVel 0")
                st.session_state.th1992.send_cmd("OUTPut1:STATe OFF")
            except Exception:
    ...
    if st.session_state.final_flow:
        ...
        st.info(f"正在进行综合测量流程：{current['name']} / {flow['phase']}")
        if flow["rows"]:
...
# -------- Tab 6: 通道切换控制 --------
with tab6:
    st.subheader("🔀 通道切换模块回路切换")
    st.markdown("点击按钮切换回路，切换后进入 **功能四** 执行 C-V 扫描即可记录该回路曲线。")
    ...
    def switch_circuit(circuit_id, label, session_name):
        if not st.session_state.relay:
            st.warning("请先在左侧连接通道切换模块")
            return
        reply = st.session_state.relay.switch_to(circuit_id)
        if relay_reply_ok(reply, circuit_id):
            st.session_state.cv_active_circuit = session_name
            st.success(f"✅ 已切换到 **{label}**（控制器回复: {reply}）")
            st.rerun()
        else:
